...
def detect_anomalies(user_id, start_time, end_time, client_id=None):
    ...
    try:
        ...
        query = LogEntry.objects.filter(
            user_id=user_id, timestamp__range=(start_time, end_time)
        )
        if client_id:
        ...
        for log, density_score in zip(logs, density_scores):
            ...
            anomaly_score = 0.5 * (entropy_score < entropy_threshold) + 0.5 * (
                density_score > density_threshold
            )
            if anomaly_score > 0:
        ...
        logger.info(
            f"Processed {len(logs)} logs for user_id={user_id}, flagged {updated_logs} anomalies"
        )
