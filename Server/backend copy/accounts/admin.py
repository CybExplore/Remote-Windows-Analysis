...
from .models import CustomUser, UserProfile, OAuth2Token, AuditLog, PasswordHistory
...
class OAuth2TokenInline(admin.TabularInline):
    model = OAuth2Token
    fields = ('access_token', 'expires_at', 'ip_address', 'is_active')
    readonly_fields = ('access_token', 'expires_at', 'ip_address', 'is_active')
    extra = 0
...
class CustomUserAdmin(UserAdmin):
    ...
    ordering = ('-created_at',)
    inlines = [UserProfileInline, OAuth2TokenInline]
