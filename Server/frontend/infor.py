            ...
            profiles.append(
                {
                    ...
                    "LastUseTime": (
                        wmi_datetime_to_iso(profile.LastUseTime)
...
def get_logged_on_sessions(username):
    ...
    for session in c.Win32_LogonSession():
        assoc_users = c.query(
            f"ASSOCIATORS OF {{Win32_LogonSession.LogonId='{session.LogonId}'}} WHERE AssocClass=Win32_LoggedOnUser Role=Dependent"
        )
                ...
                sessions.append(
                    {
                        ...
                        "StartTime": (
                            wmi_datetime_to_iso(session.StartTime)
