...
class BaseOAuth2TestCase(TestCase):
    ...
    def _login_and_authorize(self, url_func=None):
        ...
        response = self.client.post(
            self.auth_url2(), {"authorize": True, "scope": constants.SCOPES[0][1]}
        )
...
class AuthorizationTest(BaseOAuth2TestCase):
    ...
    def test_authorization_requires_client_id(self):
        ...
        self.assertTrue(
            "An unauthorized client tried to access your resources." in response.content
        )
    ...
    def test_authorization_requires_supported_response_type(self):
        ...
        response = self.client.get(
            self.auth_url()
            + "?client_id=%s&response_type=unsupported" % self.get_client().client_id
        )
        response = self.client.get(self.auth_url2())

        self.assertEqual(400, response.status_code)
        self.assertTrue(
            escape("'unsupported' is not a supported response type.")
            in response.content
        )

        response = self.client.get(
            self.auth_url()
            + "?client_id=%s&response_type=code" % self.get_client().client_id
        )
        response = self.client.get(self.auth_url2())
        ...
        response = self.client.get(
            self.auth_url()
            + "?client_id=%s&response_type=token" % self.get_client().client_id
        )
        response = self.client.get(self.auth_url2())
    ...
    def test_authorization_requires_a_valid_redirect_uri(self):
        ...
        response = self.client.get(
            ...
            % (self.get_client().client_id, self.get_client().redirect_uri + "-invalid")
        )
        response = self.client.get(self.auth_url2())

        self.assertEqual(400, response.status_code)
        self.assertTrue(
            escape("The requested redirect didn't match the client settings.")
            in response.content
        )
        ...
        response = self.client.get(
            self.auth_url()
            + "?client_id=%s&response_type=code&redirect_uri=%s"
            % (self.get_client().client_id, self.get_client().redirect_uri)
        )
        response = self.client.get(self.auth_url2())
    ...
    def test_authorization_requires_a_valid_scope(self):
        ...
        response = self.client.get(
            self.auth_url()
            + "?client_id=%s&response_type=code&scope=invalid+invalid2"
            % self.get_client().client_id
        )
        response = self.client.get(self.auth_url2())
        ...
        response = self.client.get(
            self.auth_url()
            + "?client_id=%s&response_type=code&scope=%s"
            % (self.get_client().client_id, constants.SCOPES[0][1])
        )
        response = self.client.get(self.auth_url2())
        self.assertEqual(200, response.status_code)
    ...
    def test_authorization_is_not_granted(self):
        ...
        response = self.client.post(
            self.auth_url2(), {"authorize": False, "scope": constants.SCOPES[0][1]}
        )
        self.assertEqual(302, response.status_code, response.content)
        self.assertTrue(self.redirect_url() in response["Location"])
...
class AccessTokenTest(BaseOAuth2TestCase):
    ...
    def _login_authorize_get_token(self):
        ...
        for prop in required_props:
            self.assertIn(
                prop,
                token,
                "Access token response missing " "required property: %s" % prop,
            )
    ...
    def test_refreshing_an_access_token(self):
        ...
        self.assertEqual(400, response.status_code)
        self.assertEqual(
            "invalid_grant", json.loads(response.content)["error"], response.content
        )
...
class AuthBackendTest(BaseOAuth2TestCase):
    ...
    def test_basic_client_backend(self):
        ...
        request.META["HTTP_AUTHORIZATION"] = "Basic " + "{0}:{1}".format(
            self.get_client().client_id, self.get_client().client_secret
        ).encode("base64")

        self.assertEqual(
            BasicClientBackend().authenticate(request).id,
            2,
            "Didn't return the right client.",
        )
    ...
    def test_request_params_client_backend(self):
        ...
        self.assertEqual(
            RequestParamsClientBackend().authenticate(request).id,
            2,
            "Didn't return the right client.'",
        )
