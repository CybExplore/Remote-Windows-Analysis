...
JWKValuesRegistry = {
    ...
    "RSA": {
        ...
        "dp": JWKParameter("First Factor CRT Exponent", False, False, ParmType.b64u),
        "dq": JWKParameter("Second Factor CRT Exponent", False, False, ParmType.b64u),
        "qi": JWKParameter("First CRT Coefficient", False, False, ParmType.b64u),
        "oth": JWKParameter("Other Primes Info", False, False, ParmType.unsupported),
    },
    "oct": {
...
JWKParamsRegistry = {
    ...
    "x5t": JWKParameter("X.509 Certificate SHA-1 Thumbprint", True, None, None),
    "x5t#S256": JWKParameter("X.509 Certificate SHA-256 Thumbprint", True, None, None),
}
...
# RFC 7517 - 8.2
JWKUseRegistry = {"sig": "Digital Signature or MAC", "enc": "Encryption"}
...
class InvalidJWKUsage(JWException):
    ...
    def __str__(self):
        ...
        return 'Invalid usage requested: "%s". Valid for: "%s"' % (usage, valid)
...
class InvalidJWKOperation(JWException):
    ...
    def __str__(self):
        ...
        return 'Invalid operation requested: "%s". Valid for: "%s"' % (op, valid)
...
class JWK(dict):
    ...
    def _import_pyca_pub_rsa(self, key, **params):
        ...
        params.update(kty="RSA", n=self._encode_int(pn.n), e=self._encode_int(pn.e))
        self.import_key(**params)
        ...
        if crv:
            ...
            if ccrv != cname:
                raise InvalidJWKValue(
                    'Curve requested is "%s", but ' 'key curve is "%s"' % (name, crv)
                )
        kty = self.get("kty")
        if kty is not None and ctype is not None and kty != ctype:
            raise InvalidJWKType(
                'Curve Requested is of type "%s", but '
        ...
        for name, val in JWKValuesRegistry[kty].items():
            ...
            if val.type == ParmType.b64 and name in newkey:
                ...
                try:
                    ...
                    raise InvalidJWKValue('"%s" is not base64url encoded' % name) from e
    ...
    def get_op_key(self, operation=None, arg=None):
        ...
        """
        validops = self.get("key_ops", list(JWKOperationsRegistry.keys()))
    ...
    def import_from_pyca(self, key):
        ...
        elif isinstance(key, (Ed25519PrivateKey, Ed448PrivateKey, X25519PrivateKey)):
            self._import_pyca_pri_okp(key)
        elif isinstance(key, (Ed25519PublicKey, Ed448PublicKey, X25519PublicKey)):
    ...
    def __setitem__(self, item, value):
        ...
        if item in list(JWKValuesRegistry[kty].keys()):
            ...
            if JWKValuesRegistry[kty][item].type == ParmType.b64:
                try:
                    ...
                    raise InvalidJWKValue('"%s" is not base64url encoded' % item) from e
        ...
        for name in list(JWKTypesRegistry.keys()):
            ...
            if item in list(JWKValuesRegistry[name].keys()):
                raise KeyError("Cannot set '{}' on '{}' key type".format(item, kty))
    ...
    def __eq__(self, other):
        ...
        return self.thumbprint() == other.thumbprint() and self.get("kid") == other.get(
            "kid"
        )
