...
class JWE:
        ...
        if compact:
            ...
            if "recipients" in obj:
                ...
                if "encrypted_key" in obj:
                    enc["encrypted_key"] = base64url_encode(obj["encrypted_key"])
                if "header" in obj:
        ...
        for k in crit:
            if k not in self.header_registry:
                ...
                if not self.header_registry[k].supported:
                    raise InvalidJWEData("Unsupported critical header: " '"%s"' % k)

    def _unwrap_decrypt(self, alg, enc, key, enckey, header, aad, iv, ciphertext, tag):
    ...
    def decrypt(self, key):
        ...
        if not self.plaintext:
            ...
            raise InvalidJWEData(
                "No recipient matched the provided " "key" + repr(self.decryptlog)
            )
            ...
            try:
                ...
                if "recipients" in djwe:
                    ...
                    for rec in djwe["recipients"]:
                        e = {}
                        if "encrypted_key" in rec:
                            e["encrypted_key"] = base64url_decode(rec["encrypted_key"])
                        if "header" in rec:
                            e["header"] = json_encode(rec["header"])
                        o["recipients"].append(e)
                else:
                    if "encrypted_key" in djwe:
                        o["encrypted_key"] = base64url_decode(djwe["encrypted_key"])
                    if "header" in djwe:
