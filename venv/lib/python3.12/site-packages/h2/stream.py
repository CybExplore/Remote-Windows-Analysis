...
class H2Stream:
    ...
    def send_headers(
        ...
        if (not self.state_machine.client) and is_informational_response(bytes_headers):
            if end_stream:
    ...
    def advertise_alternative_service(self, field_value: bytes) -> list[Frame]:
        ...
        """
        self.config.logger.debug(
            "Advertise alternative service of %r for %r",
            field_value,
            self,
        )
    ...
    def remotely_pushed(
        self, pushed_headers: Iterable[Header]
    ) -> tuple[list[Frame], list[Event]]:
        """
    ...
    def receive_data(
        self, data: bytes, end_stream: bool, flow_control_len: int
    ) -> tuple[list[Frame], list[Event]]:
        """
        Receive some data.
        """
    ...
    def receive_alt_svc(self, frame: AltSvcFrame) -> tuple[list[Frame], list[Event]]:
        ...
        """
        self.config.logger.debug(
            "Receive Alternative Service frame on stream %r",
            self,
        )
    ...
    def reset_stream(self, error_code: ErrorCodes | int = 0) -> list[Frame]:
        ...
        """
        self.config.logger.debug(
            "Local reset %r with error code: %d",
            self,
            error_code,
        )
    ...
    def stream_reset(self, frame: RstStreamFrame) -> tuple[list[Frame], list[Event]]:
        ...
        """
        self.config.logger.debug(
            "Remote reset %r with error code: %d",
            self,
            frame.error_code,
        )
    ...
    def _build_hdr_validation_flags(self, events: Any) -> HeaderValidationFlags:
        ...
        is_trailer = isinstance(
            events[0],
            (_TrailersSent, TrailersReceived),
        )
        ...
        is_push_promise = isinstance(
            events[0],
            (PushedStreamReceived, _PushedRequestSent),
        )
    ...
    def _build_headers_frames(
        ...
        header_blocks = [
            ...
            for i in range(
                0,
                len(encoded_headers),
                (self.max_outbound_frame_size or 0),
            )
        ]
...
def _decode_headers(
    headers: Iterable[HeaderWeaklyTyped], encoding: str
) -> Generator[HeaderTuple, None, None]:
    """
