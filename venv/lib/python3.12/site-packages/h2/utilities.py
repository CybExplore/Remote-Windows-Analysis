...
def _secure_headers(
    headers: Iterable[Header], hdr_validation_flags: HeaderValidationFlags | None
    ...
    for header in headers:
        ...
        if header[0] in _SECURE_HEADERS or (
            header[0] in b"cookie" and len(header[1]) < 20
        ):
...
def guard_increment_window(current: int, increment: int) -> int:
    ...
    if new_size > LARGEST_FLOW_CONTROL_WINDOW:
        msg = (
            f"May not increment flow control window past {LARGEST_FLOW_CONTROL_WINDOW}"
        )
...
def _reject_empty_header_names(
    headers: Iterable[Header], hdr_validation_flags: HeaderValidationFlags
) -> Generator[Header, None, None]:
    """
...
def _reject_uppercase_header_fields(
    headers: Iterable[Header], hdr_validation_flags: HeaderValidationFlags
) -> Generator[Header, None, None]:
    """
...
def _assert_header_in_set(
    bytes_header: bytes, header_set: set[bytes | str] | set[bytes] | set[str]
) -> None:
    """
...
def _reject_pseudo_header_fields(
    headers: Iterable[Header], hdr_validation_flags: HeaderValidationFlags
) -> Generator[Header, None, None]:
    """
...
def _validate_host_authority_header(
...
def _check_host_authority_header(
    headers: Iterable[Header], hdr_validation_flags: HeaderValidationFlags
) -> Generator[Header, None, None]:
    """
...
def _strip_surrounding_whitespace(
    headers: Iterable[Header], hdr_validation_flags: HeaderValidationFlags | None
) -> Generator[Header, None, None]:
    """
...
def _check_sent_host_authority_header(
    headers: Iterable[Header], hdr_validation_flags: HeaderValidationFlags
) -> Generator[Header, None, None]:
    """
...
def _split_outbound_cookie_fields(
    headers: Iterable[Header], hdr_validation_flags: HeaderValidationFlags | None
) -> Generator[Header, None, None]:
    """
...
def normalize_outbound_headers(
    ...
    should_split_outbound_cookies: bool = False,
) -> Generator[Header, None, None]:
    """
