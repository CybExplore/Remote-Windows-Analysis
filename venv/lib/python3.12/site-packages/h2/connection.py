...
class H2Connection:
    ...
    def _begin_new_stream(
        self, stream_id: int, allowed_ids: AllowedStreamIDs
    ) -> H2Stream:
        """
        ...
        self.config.logger.debug(
            "Attempting to initiate stream ID %d",
            stream_id,
        )
    ...
    def initiate_upgrade_connection(
        self, settings_header: bytes | None = None
    ) -> bytes | None:
        """
        ...
        """
        self.config.logger.debug(
            "Upgrade connection. Current settings: %s",
            self.local_settings,
        )
    ...
    def send_headers(
        ...
        stream = self._get_or_create_stream(
            stream_id,
            AllowedStreamIDs(self.config.client_side),
        )
    ...
    def send_data(
        ...
        """
        self.config.logger.debug(
            "Send data on stream ID %d with len %d",
            stream_id,
            len(data),
        )
        frame_size = len(data)
        ...
        self.config.logger.debug(
            "Frame size on stream ID %d is %d",
            stream_id,
            frame_size,
        )
    ...
    def increment_flow_control_window(
        self, increment: int, stream_id: int | None = None
    ) -> None:
        """
        ...
        if stream_id is not None:
            ...
            self.config.logger.debug(
                "Increase connection flow control window by %d",
                increment,
            )
    ...
    def push_stream(
        ...
        """
        self.config.logger.debug(
            "Send Push Promise frame on stream ID %d",
            stream_id,
        )
    ...
    def update_settings(self, new_settings: dict[SettingCodes | int, int]) -> None:
        ...
        """
        self.config.logger.debug(
            "Update connection settings to %s",
            new_settings,
        )
        self.state_machine.process_input(ConnectionInputs.SEND_SETTINGS)
    ...
    def _flow_control_change_from_settings(
        self, old_value: int | None, new_value: int
    ) -> None:
        """
    ...
    def _inbound_flow_control_change_from_settings(
        self, old_value: int | None, new_value: int
    ) -> None:
        """
    ...
    def receive_data(self, data: bytes) -> list[Event]:
        ...
        self.config.logger.trace(
            "Process received data on connection. Received data: %r",
            data,
        )
    ...
    def _receive_headers_frame(
        self, frame: HeadersFrame
    ) -> tuple[list[Frame], list[Event]]:
        """
        ...
        stream = self._get_or_create_stream(
            frame.stream_id,
            AllowedStreamIDs(not self.config.client_side),
        )
    ...
    def _receive_push_promise_frame(
        self, frame: PushPromiseFrame
    ) -> tuple[list[Frame], list[Event]]:
        """
    ...
    def _handle_data_on_closed_stream(
        self, events: list[Event], exc: StreamClosedError, frame: DataFrame
    ) -> tuple[list[Frame], list[Event]]:
    ...
    def _receive_window_update_frame(
        self, frame: WindowUpdateFrame
    ) -> tuple[list[Frame], list[Event]]:
        """
    ...
    def _receive_rst_stream_frame(
        self, frame: RstStreamFrame
    ) -> tuple[list[Frame], list[Event]]:
        """
    ...
    def _receive_priority_frame(
        self, frame: HeadersFrame | PriorityFrame
    ) -> tuple[list[Frame], list[Event]]:
        """
    ...
    def _receive_goaway_frame(
        self, frame: GoAwayFrame
    ) -> tuple[list[Frame], list[Event]]:
        """
    ...
    def _receive_alt_svc_frame(
        self, frame: AltSvcFrame
    ) -> tuple[list[Frame], list[Event]]:
        """
    ...
    def _receive_unknown_frame(
        self, frame: ExtensionFrame
    ) -> tuple[list[Frame], list[Event]]:
        """
        ...
        self.config.logger.debug(
            "Received unknown extension frame (ID %d)",
            frame.stream_id,
        )
...
def _add_frame_priority(
    frame: PriorityFrame | HeadersFrame,
    weight: int | None = None,
    depends_on: int | None = None,
    exclusive: bool | None = None,
) -> PriorityFrame | HeadersFrame:
    """
